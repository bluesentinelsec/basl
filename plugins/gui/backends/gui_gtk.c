...
static gboolean on_window_close_gtk3(GtkWidget *widget, void *event, void *data)
{
    ...
    if (cb && cb->fn) { cb->fn(cb->user_data); return 1; }
    g_vops->main_quit();
    return 0;
}
...
static void gtk3_main_loop(void)       { s_gtk3_main(); }
static void gtk3_main_quit(void)       { s_gtk3_main_quit(); }
...
{
    GtkWidget *dialog = s_gtk3_message_dialog_new(
...
{
    if (s_gtk4_loop) s_gtk4_g_main_loop_quit(s_gtk4_loop);
}
...
static void gtk4_message_box(void *parent, const char *title, const char *msg)
{
    ...
    G.g_signal_connect_data(btn, "clicked",
                            (GCallback)s_gtk4_g_main_loop_quit, loop, NULL, 1);
    G.g_signal_connect_data(win, "close-request",
                            (GCallback)s_gtk4_g_main_loop_quit, loop, NULL, 1);
...
static void gtk_be_main_loop(void)  { g_vops->main_loop(); }
static void gtk_be_quit(void)       { g_vops->main_quit(); }
...
static void *gtk_be_window_create(const char *title, int w, int h)
{
    ...
    GCallback close_handler = (g_vops == &g_gtk4_ops)
        ? (GCallback)on_window_close_gtk4
        : (GCallback)on_window_close_gtk3;
...
{
    if (handle) g_vops->window_destroy(handle);
}
...
{
    if (handle) g_vops->widget_destroy(handle);
}
