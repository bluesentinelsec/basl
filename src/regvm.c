...
#define RCASE(op) case VREG_##op:
    ...
    RCASE(NEG)
    {
        ...
        if (vigil_nanbox_is_int(R[src]))
        {
            ...
            {
                R[VREG_GET_A(i)] = regvm_encode_int(r);
                RNEXT();
            }
            goto r_overflow;
        }
    ...
    RCASE(SHL)
    {
        ...
        if (vigil_nanbox_is_uint(R[VREG_GET_B(i)]) || vigil_nanbox_is_uint(R[VREG_GET_C(i)]))
        {
            ...
            if (VIGIL_UNLIKELY(vigil_vm_checked_ushift_left(a, b, &r) != VIGIL_STATUS_OK))
                goto r_overflow;
            R[VREG_GET_A(i)] = regvm_encode_uint(r);
            RNEXT();
        }
        int64_t a = regvm_decode_int(R[VREG_GET_B(i)]);
        int64_t b = regvm_decode_int(R[VREG_GET_C(i)]);
    ...
    RCASE(SHR)
    {
        ...
        if (vigil_nanbox_is_uint(R[VREG_GET_B(i)]) || vigil_nanbox_is_uint(R[VREG_GET_C(i)]))
        {
            ...
            if (VIGIL_UNLIKELY(vigil_vm_checked_ushift_right(a, b, &r) != VIGIL_STATUS_OK))
                goto r_overflow;
            R[VREG_GET_A(i)] = vigil_nanbox_encode_uint(r);
            RNEXT();
        }
        int64_t a = regvm_decode_int(R[VREG_GET_B(i)]);
        int64_t b = regvm_decode_int(R[VREG_GET_C(i)]);
    ...
    RCASE(GET_FUNCTION)
    {
        ...
        has_reg_objects = 1; vigil_value_init_object(&R[VREG_GET_A(i)], (vigil_object_t **)&fn);
        RNEXT();
    }
    RCASE(NEW_CLOSURE)
    {
        ...
        if (status != VIGIL_STATUS_OK)
            goto r_cleanup;
        has_reg_objects = 1; vigil_value_init_object(&R[VREG_GET_A(i)], &closure);
        RNEXT();
    }
...
#define REGVM_SYNC_POST()                                                                                              \
    ...
    RCASE(NEW_INSTANCE)
    {
        ...
        if (status != VIGIL_STATUS_OK) goto r_cleanup;
        has_reg_objects = 1; vigil_value_init_object(&R[dest], &inst);
        ip++;
        RNEXT();
    }
    RCASE(NEW_ARRAY)
    {
        ...
        if (status != VIGIL_STATUS_OK) goto r_cleanup;
        has_reg_objects = 1; vigil_value_init_object(&R[dest], &arr);
        REGVM_SYNC_POST();
        RNEXT();
    }
    RCASE(NEW_MAP)
    {
        ...
        has_reg_objects = 1; vigil_value_init_object(&R[dest], &map);
        REGVM_SYNC_POST();
        RNEXT();
    }
