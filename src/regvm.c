...
#define TR_EMIT(instr)                                                                                                 \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            vs.need_release = 255;                                                                                     \
            if (_release_status != VIGIL_STATUS_OK)                                                                    \
                goto tr_fail;                                                                                          \
        }                                                                                                              \
...
#define PUSH_RESULT_REGS(preferred, count, out_base)                                                                   \
    ...
    {                                                                                                                  \
        ...
        case VIGIL_OPCODE_DEFER_CALL_VALUE: {
            ...
            for (uint32_t di = 0; di < val_count; di++)
                vs_pop(&vs);
...
static vigil_status_t regvm_drain_defers(vigil_vm_t *vm, size_t frame_idx, vigil_error_t *error)
{
    while (vm->frames[frame_idx].defer_count > 0U)
    {
        ...
        case VIGIL_VM_DEFER_CALL_NATIVE: {
            ...
            vigil_object_t *nobj =
                (nval && vigil_nanbox_has_object(*nval)) ? (vigil_object_t *)vigil_nanbox_decode_ptr(*nval) : NULL;
        ...
        case VIGIL_VM_DEFER_CALL_VALUE: {
            ...
            if (total > 0 && vm->stack_count >= total)
            {
                ...
                if (vigil_nanbox_is_object(cv))
                {
                    ...
                    if (real_args > 0)
                        memmove(&vm->stack[callee_slot], &vm->stack[callee_slot + 1],
...
/* Store a value that is known to be a primitive (i32/i64/f64/bool/nil).
   The destination register is also known to hold a primitive, so no
   object-release check is needed.  Used in typed arithmetic, comparison,
   conversion, and loop opcodes where the compiler guarantees both the
   source and destination types. */
#define RSTORE_PRIM(reg, value_expr) (R[(reg)] = (value_expr))
...
r_dispatch_switch_check:
    ...
    RCASE(LOAD_NIL)
    {
        RSTORE_PRIM(VREG_GET_A(code[ip]), VIGIL_NANBOX_NIL);
        RNEXT();
    }
    RCASE(LOAD_TRUE)
    {
        RSTORE_PRIM(VREG_GET_A(code[ip]), VIGIL_NANBOX_TRUE);
        RNEXT();
    }
    RCASE(LOAD_FALSE)
    {
        RSTORE_PRIM(VREG_GET_A(code[ip]), VIGIL_NANBOX_FALSE);
        RNEXT();
    }
    RCASE(DUP)
    {
    ...
    RCASE(ADD_I32)
    {
        ...
        if (VIGIL_UNLIKELY(VIGIL_I32_ADD_OVERFLOW(a, b, &r)))
            goto r_overflow;
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_i32(r));
        RNEXT();
    }
    RCASE(SUB_I32)
    {
    ...
    RCASE(DIV_I32)
    {
        ...
        if (VIGIL_UNLIKELY(b == 0))
            goto r_divzero;
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_i32(a / b));
        RNEXT();
    }
    RCASE(MOD_I32)
    {
    ...
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_from_bool(vigil_nanbox_decode_i32(R[VREG_GET_B(i)]) <
    ...
    RCASE(ADD_I64)
    {
        ...
        if (VIGIL_UNLIKELY(vigil_vm_checked_add(a, b, &r) != VIGIL_STATUS_OK))
            goto r_overflow;
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_int(r));
        RNEXT();
    }
    RCASE(SUB_I64)
    {
    ...
    RCASE(LT_I64)
    {
        ...
        RSTORE_PRIM(VREG_GET_A(i), (vigil_nanbox_is_uint(R[rb]) || vigil_nanbox_is_uint(R[rc2]))
    ...
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_from_bool(regvm_decode_int(R[VREG_GET_B(i)]) == regvm_decode_int(R[VREG_GET_C(i)])));
        RNEXT();
    }
    RCASE(NE_I64)
    {
    ...
    RCASE(ADD_F64)
    {
        ...
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double(a + b));
        RNEXT();
    }
    RCASE(SUB_F64)
    {
    ...
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double(sin(vigil_nanbox_decode_double(R[VREG_GET_B(i)]))));
        RNEXT();
    }
    RCASE(MATH_COS)
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double(cos(vigil_nanbox_decode_double(R[VREG_GET_B(i)]))));
        RNEXT();
    }
    RCASE(MATH_SQRT)
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double(sqrt(vigil_nanbox_decode_double(R[VREG_GET_B(i)]))));
        RNEXT();
    }
    RCASE(MATH_LOG)
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double(log(vigil_nanbox_decode_double(R[VREG_GET_B(i)]))));
        RNEXT();
    }
    RCASE(MATH_POW)
    {
        ...
        double a = vigil_nanbox_decode_double(R[VREG_GET_B(i)]);
        double b = vigil_nanbox_decode_double(R[VREG_GET_C(i)]);
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double(pow(a, b)));
        RNEXT();
    }
    ...
    RCASE(FORLOOP_I32)
    {
        ...
        if (VIGIL_UNLIKELY(VIGIL_I32_ADD_OVERFLOW(val, (int32_t)delta, &r)))
            goto r_overflow;
        RSTORE_PRIM(idx, vigil_nanbox_encode_i32(r));
        int cont = 0;
        switch (cmp)
        {
        case 0:
    ...
    RCASE(FORLOOP_I64)
    {
        ...
        if (VIGIL_UNLIKELY(vigil_vm_checked_add(val, delta, &r) != VIGIL_STATUS_OK))
            goto r_overflow;
        RSTORE_PRIM(idx, vigil_nanbox_encode_int(r));
        int cont = 0;
        switch (cmp)
        {
        case 0:
    ...
    RCASE(ADD)
    {
        ...
        {
            RSTORE(rd,
                   vigil_nanbox_encode_double(vigil_nanbox_decode_double(R[ra]) + vigil_nanbox_decode_double(R[rb])));
            RNEXT();
        }
    ...
    RCASE(LT)
    {
        ...
        if (vigil_nanbox_is_uint(R[ra]) || vigil_nanbox_is_uint(R[rb]))
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_from_bool(regvm_decode_uint(R[ra]) < regvm_decode_uint(R[rb])));
            RNEXT();
        }
        if (vigil_nanbox_is_int(R[ra]) && vigil_nanbox_is_int(R[rb]))
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_from_bool(regvm_decode_int(R[ra]) < regvm_decode_int(R[rb])));
            RNEXT();
        }
        if (vigil_nanbox_is_double(R[ra]) && vigil_nanbox_is_double(R[rb]))
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_from_bool(vigil_nanbox_decode_double(R[ra]) < vigil_nanbox_decode_double(R[rb])));
            RNEXT();
        }
        if (regvm_compare_strings(R[ra], R[rb], &compared))
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_from_bool(compared < 0));
            RNEXT();
        }
        status = VIGIL_STATUS_UNSUPPORTED;
        goto r_cleanup;
    ...
    RCASE(NEG)
    {
        ...
        if (vigil_nanbox_is_int(R[src]))
        {
            ...
            {
                RSTORE_PRIM(VREG_GET_A(i), regvm_encode_int(r));
                RNEXT();
            }
            goto r_overflow;
        }
        ...
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double(-vigil_nanbox_decode_double(R[src])));
            RNEXT();
        }
        status = VIGIL_STATUS_UNSUPPORTED;
        goto r_cleanup;
    ...
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_from_bool(R[VREG_GET_B(i)] == VIGIL_NANBOX_FALSE || R[VREG_GET_B(i)] == VIGIL_NANBOX_NIL));
        RNEXT();
    }
    RCASE(BNOT)
    {
        ...
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_int(~regvm_decode_int(R[VREG_GET_B(i)])));
            RNEXT();
        }
        status = VIGIL_STATUS_UNSUPPORTED;
        goto r_cleanup;
    ...
    {
        vigil_reg_instr_t i = code[ip];
        RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_int(regvm_decode_int(R[VREG_GET_B(i)]) & regvm_decode_int(R[VREG_GET_C(i)])));
        RNEXT();
    }
    RCASE(BOR)
    {
    ...
    RCASE(SHL)
    {
        ...
        if (vigil_nanbox_is_uint(R[VREG_GET_B(i)]) || vigil_nanbox_is_uint(R[VREG_GET_C(i)]))
        {
            ...
            if (VIGIL_UNLIKELY(vigil_vm_checked_ushift_left(a, b, &r) != VIGIL_STATUS_OK))
                goto r_overflow;
            RSTORE_PRIM(VREG_GET_A(i), regvm_encode_uint(r));
            RNEXT();
        }
        int64_t a = regvm_decode_int(R[VREG_GET_B(i)]);
        int64_t b = regvm_decode_int(R[VREG_GET_C(i)]);
    ...
    RCASE(SHR)
    {
        ...
        if (vigil_nanbox_is_uint(R[VREG_GET_B(i)]) || vigil_nanbox_is_uint(R[VREG_GET_C(i)]))
        {
            ...
            if (VIGIL_UNLIKELY(vigil_vm_checked_ushift_right(a, b, &r) != VIGIL_STATUS_OK))
                goto r_overflow;
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_uint(r));
            RNEXT();
        }
        int64_t a = regvm_decode_int(R[VREG_GET_B(i)]);
        int64_t b = regvm_decode_int(R[VREG_GET_C(i)]);
    ...
    RCASE(TO_I32)
    {
        ...
        {
            int64_t v = regvm_decode_int(R[src]);
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_i32((int32_t)v));
            RNEXT();
        }
        ...
        if (vigil_nanbox_is_double(R[src]))
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_i32((int32_t)vigil_nanbox_decode_double(R[src])));
            RNEXT();
        }
        status = VIGIL_STATUS_UNSUPPORTED;
        goto r_cleanup;
    ...
    RCASE(TO_I64)
    {
        ...
        if (vigil_nanbox_is_uint(R[src]))
        {
            RSTORE_PRIM(dst, vigil_nanbox_encode_int((int64_t)regvm_decode_uint(R[src])));
            RNEXT();
        }
        if (vigil_nanbox_is_double(R[src]))
        {
            RSTORE_PRIM(dst, vigil_nanbox_encode_int((int64_t)vigil_nanbox_decode_double(R[src])));
            RNEXT();
        }
        status = VIGIL_STATUS_UNSUPPORTED;
        goto r_cleanup;
    ...
    RCASE(TO_F64)
    {
        ...
        if (vigil_nanbox_is_double(R[src]))
        {
            RSTORE_PRIM(VREG_GET_A(i), R[src]);
            RNEXT();
        }
        if (vigil_nanbox_is_int(R[src]))
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double((double)regvm_decode_int(R[src])));
            RNEXT();
        }
        if (vigil_nanbox_is_uint(R[src]))
        {
            RSTORE_PRIM(VREG_GET_A(i), vigil_nanbox_encode_double((double)regvm_decode_uint(R[src])));
            RNEXT();
        }
        status = VIGIL_STATUS_UNSUPPORTED;
        goto r_cleanup;
...
#define REGVM_ISOLATE_CALL(arg_base_var, arg_count_val)                                                                \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            memmove(&vm->stack[_min], &vm->stack[(arg_base_var)], (size_t)(arg_count_val) * sizeof(vigil_value_t));    \
            ...
            {                                                                                                          \
                if (vigil_nanbox_has_object(vm->stack[_min + _ci]))                                                    \
                    vigil_object_retain((vigil_object_t *)vigil_nanbox_decode_ptr(vm->stack[_min + _ci]));             \
            }                                                                                                          \
            (arg_base_var) = _min;                                                                                     \
        }                                                                                                              \
    } while (0)
...
#define REGVM_STACK_HELPER(top_reg, pop_count, dst_reg, ret_count, call_expr)                                          \
    ...
    RCASE(COLLECTION_SIZE)
    {
        ...
        if (c == 1)
            ...
            REGVM_STACK_HELPER(VREG_GET_B(i), 1, VREG_GET_A(i), 1, vigil_vm_op_get_collection_size(vm, frame, error));
    }
    RCASE(GET_FIELD)
    {
        vigil_reg_instr_t i = code[ip];
    ...
    RCASE(STRING_OP)
    {
        ...
        if (status != VIGIL_STATUS_OK)
            goto r_cleanup;
        regvm_move_helper_results(vm, base, regvm_helper_result_base(str_r, pop_count), VREG_GET_A(i),
                                  helper_ret_count);
        REGVM_SYNC_POST();
        RNEXT();
    }
    ...
    RCASE(PARSE_I32)
    {
        ...
        if (s != NULL && *s != '\0')
        {
            ...
            {
                RSTORE_PRIM(dst, vigil_nanbox_encode_int((int64_t)val));
    ...
    RCASE(PARSE_F64)
    {
        ...
        if (s != NULL && *s != '\0')
        {
            ...
            {
                RSTORE_PRIM(dst, vigil_nanbox_encode_double(val));
                RSTORE((uint8_t)(dst + 1U), vigil_runtime_ok_error_value(vm->runtime));
                RNEXT();
            }
        }
    ...
    RCASE(PARSE_BOOL)
    {
        ...
        {
            RSTORE_PRIM(dst, VIGIL_NANBOX_TRUE);
            RSTORE((uint8_t)(dst + 1U), vigil_runtime_ok_error_value(vm->runtime));
            RNEXT();
        }
        if (len == 5 && memcmp(s, "false", 5) == 0)
        {
            RSTORE_PRIM(dst, VIGIL_NANBOX_FALSE);
            RSTORE((uint8_t)(dst + 1U), vigil_runtime_ok_error_value(vm->runtime));
            RNEXT();
        }
        REGVM_SYNC_PRE(src);
    ...
    RCASE(CALL_INTERFACE)
    {
        ...
        {
            vigil_error_set_literal(error, VIGIL_STATUS_INTERNAL, "regvm interface call missing operand words");
            status = VIGIL_STATUS_INTERNAL;
            goto r_cleanup;
        }
    ...
    RCASE(RETURN)
    {
        ...
        if (vm->frame_count > initial_frame_count)
        {
            ...
            {
                vigil_vm_frame_t *pf = &vm->frames[vm->frame_count - 1];
                if (pf->defers)
                {
                    free(pf->defers);
                    pf->defers = NULL;
