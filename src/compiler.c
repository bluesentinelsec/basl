                                                       ...
                                                       size_t pre_left_size)
{
    ...
    vigil_backend_lowered_fuse_last_two_get_local(state, fused);
...
vigil_status_t vigil_parser_emit_opcode(vigil_parser_state_t *state, vigil_opcode_t opcode, vigil_source_span_t span)
{
    ...
    if (opcode == VIGIL_OPCODE_TO_I64 && state->chunk.code.length > 0U)
    {
        ...
        if (vigil_opcode_i32_to_i64(last, &promoted))
        {
            ...
            vigil_backend_lowered_replace_last_opcode(state, promoted);
            return VIGIL_STATUS_OK;
        }
    ...
    if (opcode == VIGIL_OPCODE_JUMP_IF_FALSE)
    {
        ...
        if (len >= 1U)
        {
            ...
            if (fused != (vigil_opcode_t)0)
            {
                ...
                vigil_backend_lowered_replace_last_opcode(state, fused);
...
static vigil_status_t vigil_parser_patch_u32(vigil_parser_state_t *state, size_t operand_offset, uint32_t value)
{
    ...
    return vigil_backend_lowered_patch_u32(state, operand_offset, value);
}
...
vigil_status_t vigil_parser_resolve_local_symbol(vigil_parser_state_t *state, const vigil_token_t *name_token,
                                                 size_t *out_index, vigil_parser_type_t *out_type, int *out_is_capture,
                                                 size_t *out_capture_index, int *out_found)
{
    vigil_function_decl_t *decl;
    size_t local_index;
    size_t capture_index;
...
vigil_status_t vigil_parser_lookup_field(vigil_parser_state_t *state, vigil_parser_type_t receiver_type,
                                                       ...
                                                       vigil_expression_result_t *out_result)
{
    ...
    {
        size_t effective_return_count = vigil_parser_effective_call_return_count(decl->return_type, decl->return_count);

        status =
            vigil_parser_emit_call(state, call_span, defer_call, function_index, arg_count, effective_return_count);
        if (status != VIGIL_STATUS_OK)
            return status;
    }
    if (status != VIGIL_STATUS_OK)
                                                    ...
                                                    vigil_expression_result_t *out_result)
{
    ...
    {
        size_t const_idx = 0U;
        size_t effective_return_count = vigil_parser_effective_call_return_count(
            vigil_binding_type_primitive((vigil_type_kind_t)fn->return_type), fn->return_count);
                                                       ...
                                                       vigil_expression_result_t *out_result)
{
    if (method->return_count <= 1U)
    {
        ...
        {
            vigil_expression_result_set_type(out_result, vigil_parser_resolve_native_method_result_type(
                                                             state, method, class_index, method->return_type));
        }
        else
        {
            vigil_expression_result_set_type(out_result,
                                                     ...
                                                     vigil_expression_result_t *out_result)
{
    ...
    status = vigil_parser_emit_u32(
        state, (uint32_t)vigil_parser_effective_call_return_count(decl->return_type, decl->return_count),
        method_token->span);
    if (status != VIGIL_STATUS_OK)
    {
        return status;
...
static uint8_t compiler_stack_opcode_size(vigil_opcode_t opcode)
{
    static const struct
    {
        vigil_opcode_t opcode;
...
static size_t compiler_stack_op_size(const uint8_t *c, size_t ip, size_t len)
{
    ...
    size = compiler_stack_opcode_size((vigil_opcode_t)c[ip]);
    if (size > len - ip)
        return len - ip;
    return size;
}
...
static void vigil_parser_peephole_tail_call_self(vigil_parser_state_t *state, uint8_t *c, size_t len)
{
    ...
    vigil_backend_lowered_rewrite_tail_call_self(state);
}
...
static void peephole_tail_call(vigil_parser_state_t *state)
{
    ...
    if (state->expected_return_count != 1U || state->defer_emitted)
        return;
    if (len >= 18U && compiler_is_instruction_boundary(c, len - 5U, len - 18U) && c[len - 18U] == VIGIL_OPCODE_CALL &&
        c[len - 5U] == VIGIL_OPCODE_RETURN)
    {
        ...
        if (ret_count == 1U)
        {
            ...
            vigil_backend_lowered_rewrite_tail_call(state);
        }
    }
...
static void peephole_forloop_i32(vigil_parser_state_t *state, size_t loop_start)
{
...

    {
        ...
        vigil_backend_lowered_rewrite_forloop(state, loop_start, VIGIL_OPCODE_FORLOOP_I32, cmp_type);
    }
}
...
static void peephole_forloop_i64(vigil_parser_state_t *state, size_t loop_start)
{
...

    {
        ...
        c[forloop_pos + 14U] = (uint8_t)((back_off >> 24U) & 0xFF);
        state->chunk.code.length = forloop_pos + 15U;
        vigil_backend_lowered_rewrite_forloop(state, loop_start, VIGIL_OPCODE_FORLOOP_I64, cmp_type);
    }
}
...
static void vigil_parser_peephole_increment_local_i32(vigil_parser_state_t *state)
{
    ...
    vigil_backend_lowered_rewrite_increment_local(state, VIGIL_OPCODE_INCREMENT_LOCAL_I32, (int8_t)val);
}
...
static void vigil_parser_peephole_increment_local_i64(vigil_parser_state_t *state)
{
    ...
    if (state->chunk.span_count > base + 6U)
        state->chunk.span_count = base + 6U;
    vigil_backend_lowered_rewrite_increment_local(state, VIGIL_OPCODE_INCREMENT_LOCAL_I64, (int8_t)val);
}
...
static void vigil_parser_peephole_locals_i32_store(vigil_parser_state_t *state)
{
    ...
    vigil_backend_lowered_rewrite_locals_store(state, store_op);
}
    ...
    if (!t->is_global_assignment && !t->is_capture_local && vigil_parser_type_is_f64(t->target_type))
    {
        ...
        if (len >= 7U && code[len - 6U] == VIGIL_OPCODE_SET_LOCAL && code[len - 1U] == VIGIL_OPCODE_POP)
        {
            ...
            if (store_op != (vigil_opcode_t)0)
            {
                ...
                vigil_backend_lowered_rewrite_f64_store(state, store_op);
            }
        }
    }
    return VIGIL_STATUS_OK;
...
vigil_status_t vigil_compile_seed_parameter_symbols(vigil_parser_state_t *state, const vigil_function_decl_t *decl)
{
    vigil_status_t status;
    size_t i;
                                            ...
                                            const vigil_parser_state_t *parent_state)
{
    ...
    status = vigil_compile_materialize_lowered_function_body(program, decl, &body, &object);
    if (status != VIGIL_STATUS_OK)
        return status;
                                                         ...
                                                         const vigil_parser_state_t *parent_state)
{
    ...
    status = vigil_compile_backend_try_materialize_special_function(program, function_index, &handled);
    if (handled || status != VIGIL_STATUS_OK)
        return status;
