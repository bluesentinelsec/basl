...
static const char *transpile_cmake_template =
    ...
    "if(VIGIL_INCLUDE_DIR)\n"
    "    target_include_directories(vigil_app PRIVATE ${VIGIL_INCLUDE_DIR})\n"
    "endif()\n"
    "if(VIGIL_LIB_DIR)\n"
    "    find_library(VIGIL_LIB vigil HINTS ${VIGIL_LIB_DIR})\n"
    "    if(VIGIL_LIB)\n"
    "        target_link_libraries(vigil_app PRIVATE ${VIGIL_LIB})\n"
    "    endif()\n"
    "endif()\n"
...
static int transpile_write_project(const char *output_dir, const vigil_string_t *transpiled,
                                    const vigil_object_t *function, const char *script_path,
                                    vigil_runtime_t *runtime, vigil_error_t *error)
{
    ...
    /* Build vigil_main.c with embedded source path for runtime compilation. */
    vigil_string_init(&main_src, runtime);
    {
        /* Read the original source file to embed it. */
        char *src_text = NULL;
        size_t src_len = 0;
        {
            ...
            if (!src_text) { fclose(fp); return 0; }
            if (fread(src_text, 1, src_len, fp) != src_len) { free(src_text); fclose(fp); return 0; }
            src_text[src_len] = '\0';
            fclose(fp);
        }
        ...
        vigil_string_append_cstr(&main_src,
            ...
            "\nstatic const char vigil_source[] = \"", error);

        /* Escape the source text as a C string literal. */
        for (size_t i = 0; i < src_len; i++)
        {
            char esc[8];
            unsigned char ch = (unsigned char)src_text[i];
            if (ch == '"' || ch == '\\')
            ...
            vigil_string_append_cstr(&main_src, esc, error);
        }
        free(src_text);
...
static int cmd_transpile(const char *script_path, const char *output_dir)
{
    ...
    if (!transpile_write_project(output_dir, &source, function, script_path, runtime, &error))
    {
        exit_code = 1;
        goto cleanup;
    }
